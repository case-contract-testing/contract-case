...
One part of the plugin workflow is still work in progress: the
[DSL generator](./dsl-generation) can't yet be run outside the ContractCase
repository, so DSL classes for your plugin currently need to be written by
hand. See [Declaring your DSL](./dsl-generation) for details.
