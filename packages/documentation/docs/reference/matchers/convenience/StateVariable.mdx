import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

Matches the content of a variable that comes from a state.

See [state  definitions](https://case.contract-testing.io/docs/defining-contracts/http-client/state-definitions)
and [state handlers](https://case.contract-testing.io/docs/verifying-contracts/http-server/state-handlers) for
more details.

<Tabs groupId="language">
  <TabItem value="typescript" label="Typescript">
```typescript
import { matchers } from '@contract-case/case-definition-dsl'

new matchers.convenience.StateVariable(name: string)
```

#### Parameters <a name="Parameters" id="Parameters"></a>
| **Name** | **Type** | **Description** |
| --- | --- | --- |
| name | `string` | The name of the variable. |
  </TabItem>
  <TabItem value="java" label="Java">
```java
import io.contract_testing.contractcase.definitions.matchers.convenience.StateVariable;

new StateVariable(java.lang.String name);
```

#### Parameters <a name="Parameters" id="Parameters"></a>
| **Name** | **Type** | **Description** |
| --- | --- | --- |
| name | `java.lang.String` | The name of the variable. |
  </TabItem>
</Tabs>
