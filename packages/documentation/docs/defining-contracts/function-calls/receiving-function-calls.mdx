...
serialised content of the thrown error with a `payload` matcher. In Java, the
exception is serialised with Jackson (with the standard `Throwable` properties
such as the stack trace, cause and message removed), so any public getters on
your exception class become part of the payload. You can control this with
Jackson annotations on the exception class.
