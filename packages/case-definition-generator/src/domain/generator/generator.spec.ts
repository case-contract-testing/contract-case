...
`);
  ...
  describe('java', () => {
    it('generates a matcher as expected', () => {
      expect(generateJavaDslCode(matcherDefinition, 'tests', '_case').content)
        .toBe(`package io.contract_testing.contractcase.definitions.matchers.tests;
...
public class ArrayEachEntryMatches<M> {
  ...
  public ArrayEachEntryMatches(M matcher, List<Object> example) {
    ...
    it('works for the other case too', () => {
      expect(
        generateJavaDslCode(matcherContainsDefinition, 'tests', '_case')
          .content,
      ).toBe(
        `package io.contract_testing.contractcase.definitions.matchers.tests;
